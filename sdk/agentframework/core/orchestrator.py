...
class AsyncOrchestrator:
    ...
    async def run(self, flow: Flow, inputs: dict[str, Any],
        ...
        await self.state_store.record_audit_event(run.run_id, "RUN_CREATED")
        if on_created is not None:
            on_created(run.run_id)
        await self.state_store.update_run_status(run.run_id, RunStatus.RUNNING)
        await self.state_store.record_audit_event(run.run_id, "RUN_STARTED")
        if self.logger:
        ...
        try:
            for level in levels:
                ...
                for name, result in zip(level, results):
                    if isinstance(result, Exception):
                        ...
                        await self.state_store.record_audit_event(run.run_id, "RUN_FAILED")
                        if self.logger:
            ...
            await self.state_store.record_audit_event(run.run_id, "RUN_SUCCEEDED")
            if self.logger:
    ...
    async def _await_approval(self, run_id: str, task_name: str) -> None:
        ...
        await self.state_store.record_audit_event(run_id, "RUN_WAITING_APPROVAL", task_name)
        if self.logger:
        ...
        await self.state_store.record_audit_event(
            run_id, f"APPROVAL_RESOLVED:{'approved' if approved else 'rejected'}", task_name
        )
        if self.logger:
