...
Model name churns fast on Google's side (see docs/Memory.md): `gemini-2.5-flash`, this file's
original default, was retired for new users and started returning a 404 telling this exact
project's own deployed code to switch to `gemini-3.6-flash` — found via a real Render runtime
traceback, not proactively. If this happens again, the fix is almost always exactly what the
error message itself says to do; `model=` is a constructor argument specifically so a caller
hitting a similar deprecation can override it without a code change while a permanent fix lands
here.
