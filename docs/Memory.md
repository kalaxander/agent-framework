...
- **Phase C started: third reference agent — Expense Approval Agent.** User picked this over an
  ...
  - **Not yet done**: the dispatch-desk frontend (`frontend/index.html`) has no UI for the
    ...
    than a quick bolt-on).
  - **User confirmed the local `run.py` demo worked on their own machine**, then confirmed the
    live frontend's Expense Approval tab worked end to end on the real deployment — submitted a
    real expense, saw it pause, clicked Approve for real, watched it complete. This is the first
    time a human genuinely made a live, real approve/reject decision through this project's own
    UI rather than a script or curl command.
  - **Real, live production bug found on a SECOND attempt**: same expense flow failed before
    reaching approval. My frontend's error banner at the time only said a generic "Run failed
    before reaching approval — check /docs for details," hiding the actual cause — a real gap,
    fixed immediately: `errorMessage` logic extended to find the specific failed task in the
    response and surface its real `error` string directly in the banner, including the run_id,
    instead of making the user go dig through `/docs` themselves. Verified in Node against a
    simulated failure shape before trusting it.
  - **First hypothesis was wrong, second was right — worth recording both.** The improved error
    banner then showed `API key not valid`, which read at first like the just-rotated
    `GEMINI_API_KEY` hadn't propagated — a reasonable first guess, but wrong. After the user
    regenerated and reset the key, a DIFFERENT, unrelated failure appeared: a raw HTTP 500 on
    the support-ticket flow too (no run_id at all, meaning it failed before a run was even
    created) — serious enough to need Render's actual runtime logs rather than guessing further
    from the frontend alone. The real traceback showed the true cause:
    `google.genai.errors.ClientError: 404 NOT_FOUND` — `models/gemini-2.5-flash` (this file's
    hardcoded default model) had been retired by Google for new users, with Google's own error
    message directly instructing "update your code to use models/gemini-3.6-flash." Nothing
    wrong with the API key at all; the two problems were coincidentally close in time but
    causally unrelated. **Fixed** `integrations/gemini_llm_provider.py`'s default `model=`
    argument from `"gemini-2.5-flash"` to `"gemini-3.6-flash"` (still fully overridable via the
    constructor, exactly the escape hatch that made this a one-line fix rather than a redesign),
    documented in that file's own docstring that Google's model names churn fast and the fix is
    almost always exactly what their own error message says to do. Also fixed a stale demo
    label in `run_demo_real_llm.py` that would otherwise have kept printing the old, now-wrong
    model name even though the constructor's new default made the actual call use the new one.
    Confirmed via web search that `gemini-3.6-flash` is a real, current, documented model
    (not hallucinated) before trusting Google's own error message at face value — though the
    error message itself, being specific to this exact key/tier, was treated as the most
    authoritative source over general search results, which showed rapid, ongoing model-name
    churn on Google's side (3.5/3.6/3.8 all referenced as existing in quick succession).
  - **Note: the sandbox reset a SECOND time mid-session** (same as the earlier reset during the
    expense-agent build). Recovered the same way — `git clone` the user's own repo directly —
    and confirmed via `git log` it had both expected recent commits before continuing. Two
    resets in one project now; worth continuing to treat "clone the user's repo first" as the
    standard, fast, safe recovery step rather than something to be alarmed by each time.
  - Not yet confirmed by the user that the model-name fix actually resolves the issue on the
    live deployment — next step is them pushing this and retrying both flows.

- **Database integration (post Phase A/B/C): extended the existing Postgres persistence, didn't
  bolt on a new one.** User asked for a "proper" DBMS layer (normalized schema, transactions,
  concurrency, indexes, audit logging) via a generic 20-phase prompt written for an unknown
  codebase. First step was inspecting the actual repo before writing anything — this project
  already had `PostgresStateStore` (2 tables: `runs`, `task_states`) live on Render, so the real
  work was finding the gaps in what already existed, not designing from scratch.
  - **Real gap found**: `task_states` used `ON CONFLICT (run_id, task_name) DO UPDATE` — every
    retry attempt **overwrote** the previous one. The framework's own audit requirement
    (`docs/PRD.md`) couldn't actually be met — you couldn't see "what happened on attempt 1 vs
    attempt 2" from the DB. Fixed by replacing `task_states` with an append-only
    `task_executions` table (`UNIQUE(run_id, task_name, attempt)`), one row per *completed*
    attempt.
  - **Second real gap**: no durable state-transition log at all — `audit_trail()` was just a
    re-read of the (already lossy) `task_states`. Added a genuinely new `audit_logs` table
    (RUN_CREATED, TASK_STARTED, TASK_SUCCEEDED/FAILED, RUN_WAITING_APPROVAL,
    APPROVAL_RESOLVED, RUN_SUCCEEDED/FAILED) wired into the orchestrator's existing hook points
    (same places `self.logger.log(...)` already fires).
  - **Deliberately did NOT add**: a standalone `agents` table (the 3 flows are static, code-
    registered factories with no metadata beyond a name string already on `runs.flow_name` — a
    dimension table would be a join with no payoff) or a standalone `results` table (1:1 with an
    execution row, no independent lifecycle). Both were on the user's generic prompt's example
    entity list; deliberately excluded per the prompt's own Critical Rule #4 ("don't add DBMS
    features just for academic demonstration").
  - **Interface discipline**: `audit_trail()` (per-task *latest* state, used by
    `fastapi_ingress.py`'s `/v1/runs/{id}/audit` and the stdlib `RestIngress`) was kept
    byte-for-byte contract-compatible — new capability (`get_execution_history`,
    `get_audit_events`, `get_agent_statistics`) was added as new methods on the `StateStore` ABC,
    not by changing what existing methods return. Verified via the full existing 69-check smoke
    suite passing unchanged, plus 8 new checks for the added behavior.
  - **A subtlety that needed a second pass**: initially wrote `task_executions` to get a row on
    every `update_task_state` call, including the transient RUNNING write at attempt-start — that
    immediately violated the new `UNIQUE(run_id, task_name, attempt)` constraint (two writes per
    attempt). Fixed by only inserting on the terminal SUCCEEDED/FAILED call; the RUNNING call now
    only emits a TASK_STARTED audit event, not an execution row. Caught by the new smoke test for
    execution history, not by inspection — worth remembering as another instance of "real
    exercise of the code surfaces bugs review alone doesn't."
  - **Corrected an earlier assumption during analysis**: initially flagged
    `observability/audit_log.py`'s `InMemoryAuditLog` as dead code duplicating the state store's
    job. It isn't — `core/executor.py` (the Phase 1 `SyncExecutor`) genuinely uses it. Left it
    alone; the "unused" claim only held for the async/Postgres path, not the sync one.
  - **New typed error**: `core.errors.StateStoreError` (wraps asyncpg exceptions at the
    `StateStore` interface boundary; `retryable` set per-instance since a connection drop and a
    duplicate-attempt UNIQUE violation need different caller behavior).
  - **Known limitation, explicitly not fixed here**: human-in-the-loop approval waits
    (`AsyncOrchestrator._approval_events`) still live only in process memory. `runs.status =
    'waiting'` is durable; the `asyncio.Event` used to resume it is not. A Render restart while a
    run is genuinely waiting on approval orphans it. Flagged in `docs/Architecture.md`, not
    silently left undocumented — fixing it would mean either polling-based resume or a
    dedicated `pending_approvals` recovery path on startup, out of scope for this pass.
  - Verified: 69 original smoke checks + 8 new (retry history, audit events, unchanged
    `audit_trail` contract, `StateStoreError.retryable`) = 77, plus the existing 7-test pytest
    `test_flow_and_executor.py` suite and 15-test `test_server_integration.py` live-server suite,
    all passing. **Not yet verified**: `run_demo_postgres.py` against a real live Postgres
    instance (no network access to one from this sandbox, same limitation noted the first time
    `PostgresStateStore` was written) — the script itself needed no changes since it only calls
    the unchanged `get_run`/`audit_trail`/`run.tasks` surface, but running it against real
    Supabase/Neon Postgres is the actual first live verification, same pattern as the Gemini
    provider before it.
