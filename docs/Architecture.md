...
### State & Memory
- **State store** (Postgres): run records, per-attempt execution history, audit log — source of
  truth for "what happened." Schema (see `integrations/postgres_state_store.py` for the exact
  DDL, and `docs/Memory.md` for the design rationale):

  ```
  runs 1 ──────── N task_executions      (append-only: one row per completed attempt,
  runs 1 ──────── N audit_logs            not overwritten on retry — see docs/Memory.md
                                           for why the original task_states upsert lost history)
  ```

  - `runs(run_id PK, flow_name, inputs JSONB, status, created_at, updated_at)` — one row per
    agent invocation (a Flow run).
  - `task_executions(execution_id PK, run_id FK, task_name, attempt, status, result JSONB,
    error, started_at, finished_at, UNIQUE(run_id, task_name, attempt))` — one row per
    *completed* attempt (succeeded or failed); the in-progress RUNNING write isn't stored here
    (it's covered by a TASK_STARTED audit event instead) to keep the UNIQUE constraint
    meaningful. `get_run()`'s "latest state per task" view is reconstructed via
    `DISTINCT ON (task_name) ... ORDER BY attempt DESC`.
  - `audit_logs(id PK, run_id FK, task_name NULLABLE, event, ts)` — durable state-transition log
    (RUN_CREATED, TASK_FAILED, RUN_WAITING_APPROVAL, APPROVAL_RESOLVED, ...); `task_name` is
    NULL for run-level events. Independent of `task_executions` and of `audit_trail()`'s
    unchanged per-task-latest contract — see `core/state_store.py`'s module docstring for why
    these three read paths (latest state, full history, event log) are kept separate rather than
    collapsed into one.
  - No separate `agents` or `results` table: the three reference flows are static, code-
    registered factories with no metadata beyond a name string (`flow_name` already covers it),
    and `result` has no independent lifecycle from the execution row it belongs to — both would
    add a join with no query pattern that benefits from it. Revisit if agents become dynamically
    registered/configured at runtime.
  - Transactions: a task's `task_executions` insert and its matching `TASK_SUCCEEDED`/
    `TASK_FAILED` audit event are written in one transaction, so the two can never disagree about
    whether an attempt actually completed. Run-status updates are single-row and left
    unwrapped — coupling them to task-level writes would add lock contention for no consistency
    benefit at this scale.
  - Known limitation, not fixed by this schema: human-in-the-loop approval waits
    (`AsyncOrchestrator._approval_events`) live only in process memory. `runs.status = 'waiting'`
    is durable, but the `asyncio.Event` used to resume it is not — a process restart while a run
    is waiting on approval orphans that run until a future recovery mechanism is added.
