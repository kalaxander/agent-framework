...
**Database integration (append-only execution history + durable audit log):** the original
Postgres state store (2 tables: `runs`, `task_states`) had a real gap — retries **overwrote**
each other via an upsert on `(run_id, task_name)`, so the database could never show "what
happened on attempt 1 vs attempt 2." This was replaced with an append-only `task_executions`
table (`UNIQUE(run_id, task_name, attempt)`, one row per completed attempt) plus a genuine
`audit_logs` table (the old "audit trail" was just a lossy re-read of `task_states`). Every
existing interface contract (`audit_trail()`, `get_run()`) was kept unchanged, so nothing
upstream broke — see `docs/Memory.md` for the full design rationale and `docs/Architecture.md`
(§ State & Memory) for the schema.

**Live-verified:** both the Expense Approval agent's approve path *and* reject path have been
run end to end against the real deployed service (`https://agent-framework-tyeu.onrender.com`)
backed by a real Neon Postgres instance — not just locally. The approve-path run's live audit
trail actually shows a task (`assess_expense`) failing once and succeeding on retry, with both
attempts recorded intact — the exact scenario the old schema would have silently destroyed.
...
## Known limitations
Worth naming explicitly rather than leaving for a reader to discover:
- **Human-in-the-loop approval waits are process-memory-only.** A run's `waiting` status is
  durable in Postgres, but the `asyncio.Event` used to actually resume it lives only in the
  running process. If the server restarts while a run is genuinely paused on approval, that run
  is orphaned until a future recovery mechanism (e.g. a `pending_approvals` reconciliation pass
  on startup) is added. Not fixed by the database work above — noted there but deliberately out
  of scope for it.
- **This is a demo-scale system, not a production one.** Single Render instance (no horizontal
  scaling), free-tier Postgres, no auth on the REST API. Entirely appropriate for a college
  project's scope, but worth stating rather than implying otherwise.
...
simulated process restart (separate connection, same data recovered intact) — and, separately,
**confirmed live in production**: both the Expense Approval agent's approve and reject paths
have been run end to end against the deployed Render service with this Neon database attached,
including inspecting the resulting `task_executions`/`audit_logs` rows directly.
