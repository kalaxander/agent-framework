...
# --- Database integration: append-only execution history + audit events (docs/Memory.md) ---

def test_db_execution_history_preserves_every_retry_attempt():
    """The original task_states upsert overwrote each retry's row — this proves that's fixed:
    every attempt is retrievable afterward, not just the last one."""
    async def _inner():
        attempts = {"count": 0}
        ...
        flow = Flow(name="retry-history")
        flow.add_task(Task(name="a", fn=flaky,
                            retry_policy=RetryPolicy(max_attempts=3, backoff_seconds=0.01,
                                                      backoff_multiplier=1.0)))

        orchestrator = AsyncOrchestrator()
        run = await orchestrator.run(flow, inputs={})
        history = await orchestrator.state_store.get_execution_history(run.run_id, "a")
        return run.tasks["a"].result, history

    result, history = asyncio.run(_inner())
    check(result == "eventually ok", "flaky task eventually succeeds via retry")
    check(len(history) == 3, "get_execution_history returns all 3 attempts, not just the last")
    check([h.status for h in history] == [TaskStatus.FAILED, TaskStatus.FAILED,
                                           TaskStatus.SUCCEEDED],
          "execution history preserves each attempt's own outcome in order")


def test_db_audit_events_recorded_across_run_lifecycle():
    async def _inner():
        flow = Flow(name="audit-events")
        flow.add_task(Task(name="a", fn=lambda ctx: "ok"))
        orchestrator = AsyncOrchestrator()
        run = await orchestrator.run(flow, inputs={})
        return await orchestrator.state_store.get_audit_events(run.run_id)

    events = asyncio.run(_inner())
    event_names = [e.event for e in events]
    check("RUN_CREATED" in event_names and "RUN_STARTED" in event_names,
          "run-level lifecycle events are recorded")
    check("TASK_SUCCEEDED" in event_names, "task-level outcome events are recorded")
    check(event_names.index("RUN_CREATED") < event_names.index("RUN_STARTED"),
          "audit events are recorded in chronological order")


def test_db_audit_trail_contract_unchanged_by_history_addition():
    """audit_trail() (per-task LATEST state) must keep its original shape/behavior even though
    get_execution_history() now also tracks every attempt — existing callers (fastapi_ingress's
    GET /v1/runs/{id}/audit, the stdlib RestIngress) must not see any behavior change."""
    async def _inner():
        flow = Flow(name="audit-trail-unchanged")
        flow.add_task(Task(name="a", fn=lambda ctx: 1))
        flow.add_task(Task(name="b", fn=lambda ctx: 2, depends_on=["a"]))
        orchestrator = AsyncOrchestrator()
        run = await orchestrator.run(flow, inputs={})
        return await orchestrator.state_store.audit_trail(run.run_id)

    trail = asyncio.run(_inner())
    check(len(trail) == 2 and [t.name for t in trail] == ["a", "b"],
          "audit_trail still returns exactly one (latest) entry per task, in name order")


def test_db_state_store_error_carries_retryable_flag():
    from agentframework.core.errors import StateStoreError
    transient = StateStoreError("connection dropped", retryable=True)
    permanent = StateStoreError("duplicate attempt", retryable=False)
    check(transient.retryable is True and permanent.retryable is False,
          "StateStoreError.retryable is set per-instance so callers can branch on it")
...
if __name__ == "__main__":
    for fn in [
        ...
        test_db_execution_history_preserves_every_retry_attempt,
        test_db_audit_events_recorded_across_run_lifecycle,
        test_db_audit_trail_contract_unchanged_by_history_addition,
        test_db_state_store_error_carries_retryable_flag,
